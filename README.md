...
| `TICKET_COST_FITTING` | 10 | 앱 피팅 1회에 차감할 스타일 티켓. **앱의 상수와 같아야 함** |
| `DEV_BYPASS_EMAILS` | — | 티켓 무제한 개발 계정 (쉼표 구분). 앱의 `isDevBypassUser()` 와 짝 |
...
## 앱 사용자 인증과 티켓 (app-auth.js)

B2C 앱의 `/try-on` 요청에만 적용됩니다.

### API 키로는 못 막습니다

앱에 심어둔 키는 **바이너리에서 그대로 추출**됩니다(APK 디컴파일).
위젯의 `pk_` 는 허용 도메인으로 보호하는데, **앱 요청에는 Origin 헤더가 없어**
그 보호가 걸리지 않습니다. 그래서 앱 요청의 보안 경계는 **Firebase ID 토큰**입니다 —
남의 계정 토큰은 만들 수 없습니다.

### 서버가 직접 차감합니다

그동안 티켓은 앱의 AsyncStorage 가 사실상 원본이고 서버는 쳐다보지도 않았습니다.
`/try-on` 을 직접 호출하면 티켓 없이 무제한이었고, **건당 56.2원이 그대로 나갔습니다.**

```
idToken 있음 → 검증 → 잔액 확인(합성 전) → 합성 → 성공 시 트랜잭션 차감 → ticketBalance 반환
idToken 없음 → 레거시 경로. 서버는 차감하지 않고 앱이 로컬에서 차감
```

| 상황 | 응답 |
|---|---|
| 토큰이 위조·만료 | `401` |
| 티켓 부족 | `402` + `ticketBalance` |
| IP 시간당 한도 초과 | `429` + `Retry-After` |

> **구버전 앱을 막지 않습니다.** 토큰을 안 보내면 예전처럼 통과시킵니다 —
> 강제하면 업데이트 전 사용자가 전부 막힙니다. 앱 배포가 충분히 퍼진 뒤 강제로 전환하세요.

> 차감은 **성공한 뒤에만**, 트랜잭션으로 합니다. 같은 계정이 여러 기기에서 동시에 눌러도
> 한 번만 빠지지 않게 하기 위해서입니다. 차감이 실패해도 **결과는 그대로 돌려줍니다** —
> 여기서 막으면 돈은 나갔는데 사용자는 결과를 못 받습니다.

> ⚠️ `TICKET_COST_FITTING` 은 앱 상수와 **반드시 같아야 합니다.** 한쪽만 바꾸면
> 서버가 더 깎거나 덜 깎습니다.
